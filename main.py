...
def calculate_score(cards):
    """Takes a list of cards and returns the score calculated from the card"""
...
def play_game():
    ...
    while not is_game_over:
        ...
        # deal_card() function to add another card to the user_cards List. If no, then the game has ended.
            ...
            user_dealing = input("Do you want to take another card from the Deck.Type 'y' for yes or 'n' no \n")
            if user_dealing == 'y':
                user_cards.append(deal_card())
            else:
                is_game_over = True
    ...
    while computer_score < 17 and computer_score != 0:
        computer_dealing = computer_cards.append(deal_card())
        computer_score = calculate_score(computer_cards)
...
play_game()
while input("Do you want to plat a game of blackjack type 'y' for yes and 'n' for no \n") == 'y':
    clear()
    play_game()
